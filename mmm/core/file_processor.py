...
class FileProcessor:
    ...
    def process_directory(
        ...
        if continue_on_error is None:
            continue_on_error = self.config.get(
                "batch_processing.continue_on_error", False
            )
    ...
    def _process_files_parallel(
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            future_to_file = {
                executor.submit(
                    self._process_single_file, file_path, output_dir, paranoid_mode
                ): file_path
            ...
            for i, future in enumerate(as_completed(future_to_file)):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        self.console.error(
                            f"❌ {file_path.name} - Failed: {result.get('error', 'Unknown error')}"
                        )
    ...
    def _process_files_sequential(
        ...
        for i, file_path in enumerate(files):
            try:
                ...
                results["results"].append({"file": str(file_path), "result": result})
    ...
    def _process_single_file(
        self, input_file: Path, output_dir: Path, paranoid_mode: bool
    ) -> Dict[str, Any]:
    ...
    def _generate_output_path(self, input_file: Path, output_dir: Path) -> Path:
        ...
        naming_pattern = self.config.get(
    ...
    def _display_summary(self, results: Dict[str, Any]):
        ...
        success_rate = (
            (results["files_processed"] / total_files * 100) if total_files > 0 else 0
        )
...
Processing Time: {results['processing_time']:.2f} seconds
        ...
        if success_rate == 100:
            ...
            self.console.warning(
                f"⚠️  Processing completed with {results['files_failed']} failures"
            )
        else:
            self.console.error(
                f"💀 Processing failed for {results['files_failed']} files"
            )

    def create_backup_batch(
        self, directory: Path, backup_dir: Optional[Path] = None
    ) -> Dict[str, Any]:
        """Create backup of all audio files in directory"""
        if backup_dir is None:
            backup_dir = (
                directory.parent / f"{directory.name}_backup_{int(time.time())}"
            )
    ...
    def verify_batch_integrity(
        self, original_dir: Path, cleaned_dir: Path
    ) -> Dict[str, Any]:
        ...
        for cleaned_file in cleaned_files:
            try:
                ...
                original_name = cleaned_file.name.replace("_clean", "").replace(
                    "_sanitized", ""
                )
                ...
                if not original_file.exists():
                    ...
                    original_stem = cleaned_file.stem.replace("_clean", "").replace(
                        "_sanitized", ""
                    )
                    for ext in [".mp3", ".wav"]:
                ...
                if original_file.exists():
                    ...
                    verification_results["details"].append(
                        {"file": str(cleaned_file), "verification": verification}
                    )
                else:
                    verification_results["files_failed"] += 1
                    verification_results["success"] = False
                ...
                verification_results["success"] = False
                verification_results["details"].append(
                    {"file": str(cleaned_file), "error": str(e)}
                )
