...
class ConsoleManager:
    ...
    def show_progress(
        self, tasks: List[tuple], title: str = "Audio Sanitization Progress"
    ):
    ...
    def display_analysis(self, analysis: Dict[str, Any]):
        ...
        if suspicious_chunks > 0:
            threat_notes.append(
                f"{suspicious_chunks} suspicious container chunks (e.g., ID3/APE/RIFF markers)"
            )
        if watermarks_detected > 0:
            threat_notes.append(f"{watermarks_detected} watermark signatures detected")
        if tags_count > 0:
            threat_notes.append(
                f"{tags_count} metadata tags present (may carry identifiers)"
            )
        ...
        self.console.print(
            Panel(table, title="📊 Forensic Analysis", border_style="blue")
        )

    def display_results(self, results: Dict[str, Any]):
        """Display sanitization results"""
        table = Table(
            title="🧹 Sanitization Results", show_header=True, header_style="bold green"
        )
        ...
        if (
            stats.get("patterns_found", 0) > 0
            and stats.get("watermarks_detected", 0) == 0
        ):
            notes.append(
                "Pattern counts reflect spectral suppression ops; detector flagged no explicit watermarks pre-scan."
            )
        ...
        if "final_hash" in results:
            table.add_row(
                ...
                (
                    "✅ Yes"
                    if results.get("original_hash") != results.get("final_hash")
                    else "❌ No"
                ),
            )
        ...
        self.console.print(
            Panel(table, title="🎯 Mission Accomplished", border_style="green")
        )
    ...
    def display_verification(self, verification: Dict[str, Any]):
        ...
        table = Table(
            title="🔍 Verification Results", show_header=True, header_style="bold blue"
        )
        ...
        table.add_row(
            "Hash Changed", "✅ Yes" if verification.get("hash_different") else "❌ No"
        )
        ...
        table.add_row(
            "Overall Assessment",
            f"[{effectiveness_style}]{'Excellent' if effectiveness >= 95 else 'Good' if effectiveness >= 80 else 'Poor'}[/{effectiveness_style}]",
        )

        # Add context notes
        notes = []
        if (
            verification.get("new_analysis", {})
            .get("metadata", {})
            .get("suspicious_chunks")
        ):
            notes.append(
                "Threat counts include suspicious container/metadata chunks; detector reported zero watermarks."
            )
        if verification.get("new_analysis", {}).get("watermarks", {}).get(
            "detected"
        ) in ([], None):
            notes.append(
                "No detector-verified watermarks; remaining threats are metadata/binary anomalies."
            )
        if notes:
            table.add_row("Notes", notes[0])
            for n in notes[1:]:
        ...
        self.console.print(
            Panel(table, title="✅ Quality Assurance", border_style="blue")
        )
    ...
    def display_detailed_analysis(self, analysis: Dict[str, Any]):
        ...
        if "statistical" in analysis:
            ...
            stat_table.add_row(
                "AI Probability", f"{stat_analysis.get('ai_probability', 0):.1%}"
            )
            stat_table.add_row(
                "Human Confidence", f"{stat_analysis.get('human_confidence', 0):.1%}"
            )
            ...
            if anomalies:
                ...
                for anomaly in anomalies[:5]:  # Show first 5
                    stat_table.add_row(
                        f"  └─ {anomaly['type']}", f"Severity: {anomaly['severity']}"
                    )
    ...
    def display_turbo_analysis(self, results: Dict[str, Any]):
        ...
        self.console.print("\n🚀" + "=" * 58 + "🚀")
        self.console.print(
            "🔥 TURBO ANALYSIS RESULTS - GPU ACCELERATED", style="bold yellow"
        )
        ...
        meta_table = Table(
            title="📋 Metadata Analysis", show_header=True, header_style="bold cyan"
        )
        ...
        meta_table.add_row(
            "Suspicious Chunks", str(len(metadata.get("suspicious_chunks", [])))
        )
        ...
        gpu_table.add_row(
            "Chunks Processed", str(gpu_watermarks.get("chunks_processed", 0))
        )
        gpu_table.add_row(
            "Watermarks Detected", str(gpu_watermarks.get("total_count", 0))
        )
        gpu_table.add_row(
            "Average Confidence", f"{gpu_watermarks.get('avg_confidence', 0):.1%}"
        )
        ...
        perf_table = Table(
            title="⚡ Performance Metrics", show_header=True, header_style="bold yellow"
        )
        ...
        perf_table.add_row(
            "Processing Time", f"{performance.get('processing_time', 0):.2f}s"
        )
        perf_table.add_row(
            "Real-time Factor", f"{performance.get('realtime_factor', 0):.1f}x"
        )
        perf_table.add_row(
            "Avg Chunk Time", f"{performance.get('avg_chunk_time', 0):.3f}s"
        )
        ...
        threat_color = (
            "red" if total_threats > 20 else "yellow" if total_threats > 10 else "green"
        )
        threat_level = (
            "🔴 VERY HIGH"
            if total_threats > 20
                ...
                "🟠 HIGH"
                if total_threats > 10
                else "🟡 MEDIUM" if total_threats > 5 else "🟢 LOW"
            )
        )
